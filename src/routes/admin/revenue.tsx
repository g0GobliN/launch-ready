...
function GrowthBadge({ value }: { value: number | null }) {
  ...
  return value > 0 ? (
    <span className="inline-flex items-center gap-0.5 text-xs font-medium text-emerald-400">
      <ChevronUp className="h-3.5 w-3.5" />+{value}%
    </span>
  ) : (
...
function AdminRevenue() {
  ...
  // Ledger
  const availableYears = Array.from(new Set(monthlyDetail.map((r) => r.isoMonth.slice(0, 4)))).sort(
    (a, b) => b.localeCompare(a),
  );
  const filteredDetail =
    yearFilter === "all"
      ? monthlyDetail
      : monthlyDetail.filter((r) => r.isoMonth.startsWith(yearFilter));
  ...
  const pagedDetail = sortedDetail.slice(
    (ledgerPage - 1) * LEDGER_PAGE_SIZE,
    ledgerPage * LEDGER_PAGE_SIZE,
  );

  function changeYearFilter(y: string) {
    setYearFilter(y);
    setLedgerPage(1);
  }
  function toggleSort() {
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-0.5 text-sm text-muted-foreground">
          Monthly recurring revenue and plan performance.
        </p>
      </div>
        ...
        <div className="grid gap-3 sm:grid-cols-3">
                ...
                <div className="space-y-2 mt-3">
                  <div>
                    <div className="flex justify-between text-xs text-muted-foreground mb-1">
                      ...
                      <span className="font-medium text-foreground">{revPct}%</span>
                    </div>
                    <div className="h-1.5 w-full rounded-full bg-muted overflow-hidden">
                      <div
                        className="h-full rounded-full transition-all"
                        style={{ width: `${revPct}%`, backgroundColor: color }}
                      />
                    </div>
                  </div>
                  <div>
                    <div className="flex justify-between text-xs text-muted-foreground mb-1">
                      ...
                      <span className="font-medium text-foreground">{userPct}%</span>
                    </div>
                    <div className="h-1.5 w-full rounded-full bg-muted overflow-hidden">
                      <div
          ...
          {rankedPlans.every((p) => p.users === 0) && (
            <div className="col-span-3 py-8 text-center text-sm text-muted-foreground">
              No paid users yet — plan rankings will appear here.
            </div>
          )}
        </div>
      </div>
      ...
      <div className="rounded-xl border border-border bg-card p-5">
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="flex gap-1">
            {MONTH_OPTIONS.map((o) => (
              <button
                key={o}
                onClick={() => setChartMonths(o)}
                className={`rounded px-2 py-0.5 text-xs font-medium transition cursor-pointer ${chartMonths === o ? "bg-primary text-primary-foreground" : "bg-muted text-muted-foreground hover:bg-muted/60"}`}
              >
                {o}M
              </button>
            ))}
        ...
        <div className="h-56">
          <ResponsiveContainer width="100%" height="100%">
            <AreaChart
              data={sparseMonths(mrrByMonth.slice(-chartMonths), chartMonths)}
              ...
              <div key={p.plan}>
                ...
                <div className="h-2 w-full rounded-full bg-muted overflow-hidden">
                  <div
                    className="h-full rounded-full transition-all"
                    style={{
                      width: `${totalPaid > 0 ? (p.users / totalPaid) * 100 : 0}%`,
      ...
      <div className="rounded-xl border border-border bg-card overflow-hidden">
        <div className="flex flex-wrap items-center justify-between gap-3 px-5 py-4 border-b border-border">
          ...
          <div className="flex flex-wrap items-center gap-2">
            <select
              ...
              className="rounded-md border border-border bg-card px-2.5 py-1 text-xs text-foreground focus:outline-none focus:ring-1 focus:ring-primary cursor-pointer"
            >
            ...
            <button
              onClick={toggleSort}
              className="inline-flex items-center gap-1 rounded-md border border-border px-3 py-1 text-xs text-muted-foreground hover:bg-muted transition cursor-pointer"
            >
              {sortDesc ? (
                <>
                  <ChevronDown className="h-3.5 w-3.5" />
        ...
        {yearTotals && (
          <div className="flex flex-wrap items-center gap-6 px-5 py-3 bg-primary/5 border-b border-border text-xs">
            ...
            <span className="text-muted-foreground">
              New paid users:{" "}
              <span className="font-medium text-foreground">+{yearTotals.totalNewPaid}</span>
            </span>
            <span className="text-muted-foreground">
              Peak MRR:{" "}
              <span className="font-medium text-foreground">
              ...
              {pagedDetail.map((row) => (
                <tr key={row.isoMonth} className="hover:bg-muted/20 transition-colors">
                  ...
                  <td className="px-4 py-3 text-xs text-muted-foreground">
                    <span className="font-medium text-foreground">{row.totalPaidUsers}</span> total
                  </td>
                  <td className="px-4 py-3 text-xs">
                    {row.newPaidUsers > 0 ? (
                      <span className="text-emerald-400 font-medium">+{row.newPaidUsers}</span>
                    ) : (
        ...
        {totalLedgerPages > 1 && (
          <div className="flex items-center justify-between border-t border-border px-5 py-3">
            <span className="text-xs text-muted-foreground">
              ...
              {Math.min(ledgerPage * LEDGER_PAGE_SIZE, sortedDetail.length)} of{" "}
              {sortedDetail.length} months
            </span>
            <div className="flex items-center gap-1">
              <button
                onClick={() => setLedgerPage(ledgerPage - 1)}
                disabled={ledgerPage <= 1}
                className="rounded-md border border-border p-1.5 text-muted-foreground hover:bg-muted disabled:opacity-40 disabled:cursor-not-allowed cursor-pointer transition"
              >
              ...
              {Array.from({ length: Math.min(5, totalLedgerPages) }, (_, i) => {
                ...
                return (
                  <button
                    ...
                    className={`min-w-[28px] rounded-md border px-2 py-1 text-xs transition cursor-pointer ${p === ledgerPage ? "border-primary bg-primary text-primary-foreground" : "border-border hover:bg-muted text-muted-foreground"}`}
                  >
                    {p}
                  </button>
                );
