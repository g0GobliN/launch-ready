...
function AdminUsers() {
  ...
  function setFilter(plan: string) {
    navigate({ search: (prev) => ({ ...prev, plan, page: 1 }) });
  }
  ...
  async function handleGrant() {
    ...
    setGranting(true);
    try {
      await grantAdminCreditsFn({
        data: { githubLogin: grantTarget, amount: parseInt(grantAmount, 10) },
      });
      setGrantTarget(null);
      window.location.reload();
    } catch (e) {
        ...
        <div className="flex gap-1">
          {["all", "free", "starter", "pro", "agency"].map((p) => (
            <button
              key={p}
              onClick={() => setFilter(p)}
              className={`rounded-full px-3 py-1 text-xs font-medium transition cursor-pointer ${(search.plan ?? "all") === p ? "bg-primary text-primary-foreground" : "bg-muted text-muted-foreground hover:bg-muted/80"}`}
            >
                ...
                return (
                  <tr key={u.github_login} className="hover:bg-muted/20 transition-colors">
                    <td className="px-5 py-3">
                      ...
                      >
                        <img
                          src={`https://avatars.githubusercontent.com/${u.github_login}?s=32`}
                    ...
                    <td className="px-4 py-3">
                      <span
                        className={`rounded-full px-2.5 py-0.5 text-xs font-semibold ${PLAN_COLORS[u.plan] ?? PLAN_COLORS.free}`}
                      >
                        {planDef.name}
                      </span>
                    </td>
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-xs text-muted-foreground">
                          {u.monthly_scan_used}/{u.monthly_scan_limit}
                        </span>
                      </div>
                    </td>
                    ...
                    <td className="px-4 py-3">
                      <button
                        onClick={() => setGrantTarget(u.github_login)}
                        className="inline-flex items-center gap-1 rounded-md border border-border px-2.5 py-1 text-xs hover:bg-muted transition cursor-pointer"
                      >
              ...
              {Array.from({ length: Math.min(5, totalPages) }, (_, i) => {
                ...
                return (
                  <button
                    key={p}
                    onClick={() => setPage(p)}
                    className={`min-w-[28px] rounded-md border px-2 py-1 text-xs transition cursor-pointer ${p === page ? "border-primary bg-primary text-primary-foreground" : "border-border hover:bg-muted text-muted-foreground"}`}
                  >
                    {p}
                  </button>
                );
        ...
        <DialogContent className="sm:max-w-sm">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
              <Zap className="h-4 w-4 text-primary" /> Grant AI Credits
            </DialogTitle>
          </DialogHeader>
          <div className="space-y-3 py-2">
            <p className="text-sm text-muted-foreground">
              Adding credits to <strong className="text-foreground">@{grantTarget}</strong>
            </p>
            <Input
              type="number"
              min={1}
        ...
        <DialogContent className="sm:max-w-sm">
          ...
          <p className="text-sm text-muted-foreground py-2">
            {adminConfirm?.makeAdmin ? (
              <>
                Give <strong className="text-foreground">@{adminConfirm.githubLogin}</strong> full
                admin access to this dashboard?
              </>
            ) : (
              <>
                Remove admin access from{" "}
                <strong className="text-foreground">@{adminConfirm?.githubLogin}</strong>?
              </>
            )}
          </p>
          <DialogFooter>
            <Button variant="outline" onClick={() => setAdminConfirm(null)}>
              Cancel
            </Button>
            <Button
              variant={adminConfirm?.makeAdmin ? "default" : "destructive"}
