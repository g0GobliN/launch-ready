...
async function requireAdmin(login: string | undefined) {
  ...
  const bootstrapAdmins = (process.env.ADMIN_GITHUB_LOGIN ?? "").split(",").map((s) => s.trim()).filter(Boolean);
  ...
  const { data } = await db.from("user_credits").select("is_admin").eq("github_login", login).maybeSingle();
...
function lastNDays(n: number) {
  ...
  for (let i = n - 1; i >= 0; i--) {
    ...
    days.push({ date: d.toISOString().split("T")[0], label: d.toLocaleDateString("en-US", { month: "short", day: "numeric" }) });
  }
  return days;
}
...
export const loadAdminOverviewFn = createServerFn({ method: "GET" })
  ...
  const [{ data: users }, { data: allScans }, { data: allJobs }] = await Promise.all([
    ...
    db.from("fix_requests").select("id, status, credits_cost, created_at").gte("created_at", since),
  ]);

  const days = lastNDays(data.days);
  const signupsByDay = days.map(({ date, label }) => ({ date: label, signups: (users ?? []).filter((u) => u.created_at.startsWith(date)).length }));
  const scansByDay = days.map(({ date, label }) => ({ date: label, scans: (allScans ?? []).filter((s) => s.created_at.startsWith(date)).length }));
  const jobsByDay = days.map(({ date, label }) => ({ date: label, jobs: (allJobs ?? []).filter((j) => j.created_at.startsWith(date)).length }));
  ...
  const mrr = (users ?? []).reduce((sum, u) => sum + (PLANS[u.plan as keyof typeof PLANS]?.priceYen ?? 0), 0);
  ...
  return {
    ...
    stats: { totalUsers, paidUsers, mrr, scansThisMonth, jobsThisMonth, completedJobs, failedJobs, conversionRate: totalUsers > 0 ? Math.round((paidUsers / totalUsers) * 100) : 0 },
  };
});

export const loadAdminUsersFn = createServerFn({ method: "GET" })
  .inputValidator(z.object({ page: z.number().default(1), search: z.string().default(""), plan: z.string().default("all") }))
  .handler(async ({ data }) => {
    const { getStoredUser } = await import("../github-token.server");
    await requireAdmin(getStoredUser()?.login);
    ...
    let query = db.from("user_credits").select("*", { count: "exact" }).order("created_at", { ascending: false });
...
export const loadAdminJobsFn = createServerFn({ method: "GET" })
  .inputValidator(z.object({ page: z.number().default(1), status: z.string().default("all"), search: z.string().default("") }))
  .handler(async ({ data }) => {
    const { getStoredUser } = await import("../github-token.server");
    await requireAdmin(getStoredUser()?.login);
    ...
    let query = db.from("fix_requests").select("id, owner_login, status, credits_cost, created_at, repos(name)", { count: "exact" }).order("created_at", { ascending: false });
...
export const loadAdminRevenueFn = createServerFn({ method: "GET" })
  ...
  const planRevenue = (["starter", "pro", "agency"] as const).map((planId) => {
    ...
    return { plan: PLANS[planId].name, users: count, mrr: count * PLANS[planId].priceYen, color: { starter: "#22c55e", pro: "#3b82f6", agency: "#f59e0b" }[planId] };
  });
  ...
  type MonthRow = { month: string; isoMonth: string; newPaidUsers: number; totalPaidUsers: number; newMrr: number; totalMrr: number; momGrowth: number | null };
  ...
  for (let i = data.months - 1; i >= 0; i--) {
    ...
    const newPaid = (users ?? []).filter((u) => u.plan !== "free" && u.created_at >= start && u.created_at <= end);
    const cumulativePaid = (users ?? []).filter((u) => u.plan !== "free" && u.created_at <= end);
    const newMrr = newPaid.reduce((s, u) => s + (PLANS[u.plan as keyof typeof PLANS]?.priceYen ?? 0), 0);
    const totalMrr = cumulativePaid.reduce((s, u) => s + (PLANS[u.plan as keyof typeof PLANS]?.priceYen ?? 0), 0);

    mrrByMonth.push({ month: label, mrr: totalMrr });
    monthlyDetail.push({ month: label, isoMonth, newPaidUsers: newPaid.length, totalPaidUsers: cumulativePaid.length, newMrr, totalMrr, momGrowth: null });
  }
  ...
  for (let i = 1; i < monthlyDetail.length; i++) {
    ...
    monthlyDetail[i].momGrowth = prev > 0 ? Math.round(((curr - prev) / prev) * 100) : curr > 0 ? 100 : 0;
  }
...
export const runPromotionFn = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    let query = db.from("user_credits").select("github_login, balance, ai_credits_total").neq("plan", "free");
    ...
    if (data.type === "bonus_credits") {
      ...
      await db.from("user_credits").update({ monthly_scan_used: 0, updated_at: new Date().toISOString() }).in("github_login", logins);
    }
...
export const grantAdminCreditsFn = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    const { data: row } = await db.from("user_credits").select("balance, ai_credits_total").eq("github_login", data.githubLogin).single();
    ...
    await db.from("credit_transactions").insert({
      id: crypto.randomUUID(), github_login: data.githubLogin, amount: data.amount, reason: "Admin grant", type: "grant",
    });
...
export const toggleAdminFn = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    if (caller?.login === data.githubLogin && !data.makeAdmin) throw new Error("You cannot revoke your own admin access.");

    const { getServiceRoleClient } = await import("../supabase.server");
    const db = getServiceRoleClient();
    const { error } = await db.from("user_credits").update({ is_admin: data.makeAdmin, updated_at: new Date().toISOString() }).eq("github_login", data.githubLogin);
